Link: https://tryhackme.com/room/nmap (Premium Room)

<img width="600" height="600" alt="image" src="https://github.com/user-attachments/assets/8abd25be-e05c-49a8-8550-55cff17aca3a" />
...
1. _It’s time to find out who is listening on the network._

Answer: No answer needed

-----------------------------------------------------------------------------------------------------------------------------------------------------

**Module 2: Host Discovery: Who Is Online**

<img width="1140" height="800" alt="image" src="https://github.com/user-attachments/assets/606172ec-8e3c-4108-9a59-d436ba883509" />
...
<img width="1228" height="1026" alt="image" src="https://github.com/user-attachments/assets/34e90ba9-3f55-4e20-b9b7-ba5cd91c2066" />
...
1. _What is the last IP address that will be scanned when your scan target is `192.168.0.1/27`?_

Answer: `192.168.0.31`

**_Explanation:_**

We type the following command on terminal `nmap -sL 192.168.0.1/27`

And we the last IP as the following:

<img width="808" height="895" alt="image" src="https://github.com/user-attachments/assets/95d1821b-94ac-4641-9bca-a102f6bc00ca" />

-------------------------------------------------------------------------------------------------------------------------------------------------

**Module 3: Port Scanning: Who Is Listening**

<img width="1200" height="1130" alt="image" src="https://github.com/user-attachments/assets/8b1bdca8-5d1e-4479-b98b-0d62cb51dea6" />
...
<img width="1199" height="715" alt="image" src="https://github.com/user-attachments/assets/6e4e818d-a6c8-4f03-a593-371a381cd22a" />
...
<img width="1199" height="715" alt="image" src="https://github.com/user-attachments/assets/399d0a20-512f-4b6e-a85f-2f4a486075e4" />
...
<img width="1220" height="735" alt="image" src="https://github.com/user-attachments/assets/21f3e165-b72a-4962-b336-83d61cfc23e1" />
...
1. _How many TCP ports are open on the target system at `MACHINE IP`?_
...
`nmap -T5 -sS -p- 10.10.221.232`

<img width="977" height="548" alt="image" src="https://github.com/user-attachments/assets/c33b901e-4f59-41da-9682-ff16008b35c5" />


2. _Find the listening web server on `MACHINE IP` and access it with your browser. What is the flag that appears on its main page?_

Answer: `THM{SECRET_PAGE_38B9P6}`

**_Explanation:_**

From the earlier question we find port `8008` listening the web server and we access the web server via `MACHINE IP:8008` and we can find the flag 

<img width="1918" height="1003" alt="image" src="https://github.com/user-attachments/assets/a00a867b-f498-47ff-84fe-38309289f668" />


------------------------------------------------------------------------------------------------------------------------------------------------

**Module 4: Version Detection: Extract More Information**

<img width="920" height="560" alt="image" src="https://github.com/user-attachments/assets/c0d5ef79-ec2d-4164-849a-70c7203546d8" />
...
| `-Pn`  | Scan hosts that appear to be down (skip host discovery)       |

**Answer the questions below**

`What is the name and detected version of the web server running on `10.10.221.232`?`

Answer: `lighttpd 1.4.74`

**_Explanation_**

We use this command `nmap -sV -p 8008 10.10.221.232` 

<img width="1139" height="290" alt="image" src="https://github.com/user-attachments/assets/c2013a2d-ee58-467f-9789-e8d9a3c2e2ea" />

------------------------------------------------------------------------------------------------------------------------------------------------

**Module 5: Timing: How Fast is Fast**

<img width="1140" height="800" alt="5f04259cf9bf5b57aed2c476-1723711770591" src="https://github.com/user-attachments/assets/db4dfca1-2956-4be3-9035-5fcdb5633daf" />
...
In the Nmap scans below, we launch a SYN scan targeting the 100 most common TCP ports,  `nmap -sS 10.10.221.232 -F`. We repeated the scan with different timings: T0, T1, T2, T3, and T4. In our lab setup, Nmap took different amounts of time to scan the 100 ports. The table below should give you an idea, but you will get different results depending on the network setup and target system.
...
<img width="1200" height="420" alt="image" src="https://github.com/user-attachments/assets/67797889-2f0a-488c-b562-015f15f4793a" />

In the screenshot below, Nmap waited 15 seconds between every two ports when we set the timing to `T1`.

<img width="1200" height="425" alt="image" src="https://github.com/user-attachments/assets/fa3b8bb9-7488-4396-bda9-b2238f5f7fbf" />

Then, the waiting dropped to 0.4 seconds for `T2` as shown below.

<img width="1200" height="425" alt="image" src="https://github.com/user-attachments/assets/a801584e-d8c3-4b92-9dc7-9f063853ef10" />
...
<img width="1200" height="425" alt="image" src="https://github.com/user-attachments/assets/969908fa-6f06-47b5-835b-95ae94680f82" />
...
_What is the non-numeric equivalent of -T4?_

Answer: `-T aggressive`

-----------------------------------------------------------------------------------------------------------------------------------------------

**Module 6: Output: Controlling What You See**
...
1. _What option must you add to your nmap command to enable debugging?_

Answer: `-d`

-----------------------------------------------------------------------------------------------------------------------------------------------

**Module 7: Conclusion and Summary**
...
|-------------------|-----------------------------------------------------------|-----------------------------------------------------------------------------------------------|
| **Target Listing** | `-sL`                                                     | List scan – list targets without scanning                                                    |
...
|                   | `-A`                                                      | OS detection, version detection, and other additions                                        |
| **Timing**        | `-T<0-5>`                                                 | Timing template – paranoid (0), sneaky (1), polite (2), normal (3), aggressive (4), insane (5) |
|                   | `--min-parallelism <numprobes>` and `--max-parallelism <numprobes>` | Minimum and maximum number of parallel probes                                               |
|                   | `--min-rate <number>` and `--max-rate <number>`           | Minimum and maximum rate (packets/second)                                                  |
|                   | `--host-timeout`                                          | Maximum amount of time to wait for a target host                                           |
| **Real-time Output** | `-v`                                                    | Verbosity level (e.g., `-vv`, `-v4`)                                                      |
|                   | `-d`                                                      | Debugging level (e.g., `-d`, `-d9`)                                                       |
| **Report Output** | `-oN <filename>`                                          | Normal output                                                                              |
|                   | `-oX <filename>`                                          | XML output                                                                                |
...
_What kind of scan will Nmap use if you run nmap 10.10.221.232 with local user privileges?_
