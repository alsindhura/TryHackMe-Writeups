...
_Please ensure you have finished the Networking Core Protocols room at minimum._
...
![image](https://github.com/user-attachments/assets/ee5a9b3b-15d6-4276-acd5-11fe2c14e839)
...
_What is the protocol name that TLS upgraded and built upon?_

Answer: `SSL`

_Which type of certificates should not be used to confirm the authenticity of a server?_
...
![image](https://github.com/user-attachments/assets/709ab9fc-795d-4c7d-92fd-f864c9a3abcc)
...
![image](https://github.com/user-attachments/assets/67d2f78f-656b-408f-8a62-2ad0c33fd04d)
...
![image](https://github.com/user-attachments/assets/7c559b1e-1383-4e0f-807c-0fd83258f9d6)
...
![image](https://github.com/user-attachments/assets/5398f6ca-fbf8-46c7-8ebe-a7a34f1e6f26)
...
Wireshark displaying a TCP connection getting established, a TLS session getting established, and HTTP request sent and response received.

![image](https://github.com/user-attachments/assets/3f74715b-087f-4505-a291-43ceb7412379)

If you want to see the data exchanged, now is your chance! It is still regular HTTP traffic hidden from prying eyes.

![image](https://github.com/user-attachments/assets/4871edf1-4f8d-4b7d-b315-77ba1e597bbb)
...
_How many packets did the TLS negotiation and establishment take in the Wireshark HTTPS screenshots above?_

Answer: `8`

Hint" `You need to count the packets to establish a TCP connection and the packets to negotiate the TLS session.`

_What is the number of the packet that contain the GET /login when accessing the website over HTTPS?_
...
We refer to image below and see `GET /login` "Time" as 10

![image](https://github.com/user-attachments/assets/71291fc3-327e-462a-be6b-9c8885ab7d93)

-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

**Module 4: SMTPS, POP3S, and IMAPS**
...
_If you capture network traffic, in which of the following protocols can you extract login credentials: SMTPS, POP3S, or IMAP?_

Answer: `IMAP`

**_Explanation:_**

| Protocol | Can Capture Email/Login Credentials? | Reason                                                                                                                             |
|----------|-------------------------------------|------------------------------------------------------------------------------------------------------------------------------------|
| IMAP     | Yes                                 | Maintains a persistent connection and syncs mailbox in real-time, transmitting credentials and emails multiple times if unencrypted. |
| POP3     | Generally No                        | Downloads emails and disconnects quickly, reducing interception opportunity; credentials sent once per session.                   |
| SMTP     | Generally No                        | Used mainly for sending mail; credentials sent for authentication, but capturing incoming emails isn't applicable.                |
...
- **_Secure authentication_**: Besides password-based authentication, SSH supports public key and two-factor authentication.

- **_Confidentiality_**: OpenSSH provides end-to-end encryption, protecting against eavesdropping. Furthermore, it notifies you of new server keys to protect against man-in-the-middle attacks.

- **_Integrity_**: In addition to protecting the confidentiality of the exchanged data, cryptography also protects the integrity of the traffic.

- **_Tunneling_**: SSH can create a secure “tunnel” to route other protocols through SSH. This setup leads to a VPN-like connection.

- **_X11 Forwarding_**: If you connect to a Unix-like system with a graphical user interface, SSH allows you to use the graphical application over the network.
...
![image](https://github.com/user-attachments/assets/f91174f6-466a-454c-addd-59a84d3cdcd6)

While the TELNET server listens on port 23, the SSH server listens on port 22.

**Answer the questions below**

_What is the name of the open-source implementation of the SSH protocol?_

Answer: `OpenSSH`

----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

**Module 6: SFTP and FTPS**
...
| Protocol | Full Name              | Port(s)           | Encryption           | Description                                                                                  |
|----------|-----------------------|-------------------|----------------------|----------------------------------------------------------------------------------------------|
| FTP      | File Transfer Protocol | 21 (control), 20 (data) | None                 | Standard protocol for transferring files; data and credentials sent **unencrypted**, insecure. |
| FTPS     | FTP Secure (FTP over SSL/TLS) | 21 (control), 989/990 (data) | SSL/TLS               | Adds encryption via SSL/TLS to FTP; secures credentials and data transfers.                   |
| SFTP     | SSH File Transfer Protocol | 22                | Encrypted via SSH     | Completely different protocol built on SSH; encrypts commands and data; more secure and firewall-friendly. |
...
Once we click on "View Site" we see something like below

![image](https://github.com/user-attachments/assets/b44b3d04-91b1-4f1f-a5c9-a82777bca12e)

And when we click on "Start" we map the insecure protocols with the secure ones:

![proc](https://github.com/user-attachments/assets/bceb8509-1977-48d3-98a4-1517ead34e28)

Mapping answer:

| Insecure Protocol | Insecure Port | → Secure Port   | Secure Protocol |
|-------------------|---------------|-----------------|-----------------|
| HTTP              | 80            | → 443           | HTTPS           |
...
Once our answers are correct, we see the flag

![image](https://github.com/user-attachments/assets/5b1dc55c-e2c7-42d9-8f6e-27882afe4e00)
...
![image](https://github.com/user-attachments/assets/64cf878c-3a8b-4319-b4d1-95063dda7b0a)
...
![image](https://github.com/user-attachments/assets/35fdd172-18e5-4ae8-b5e3-feaaef5af9c1)
...
![image](https://github.com/user-attachments/assets/146f6759-072c-4a16-a3fe-1c79ad12038e)
...
_What would you use to connect the various company sites so that users at a remote office can access resources located within the main branch?_
...
![image](https://github.com/user-attachments/assets/f41f99bf-64db-489d-8dc4-b58b4715fa35)
